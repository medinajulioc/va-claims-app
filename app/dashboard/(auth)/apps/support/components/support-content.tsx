...
import { SupportHeader, SupportBubble, SupportFooter, SupportUserDetailSheet } from "./";
...
export function SupportContent() {
  ...
  return (
    <div className="bg-background fixed inset-0 z-50 flex h-full flex-col p-4 lg:relative lg:z-10 lg:bg-transparent lg:p-0">
      <SupportHeader user={selectedChat.user} />
          ...
          <div className="flex flex-col items-start space-y-10 py-8">
            {!hasMessages && (
              <div className="mx-auto my-6 text-center">
                <p className="text-muted-foreground mb-2">
                  👋 Hi {currentUserFirstName}! Welcome to your support chat with{" "}
                  {recipientFirstName}.
                </p>
            ...
            {hasMessages &&
              messages.map((item: ChatMessageProps, key) => (
                <SupportBubble message={item} type={item.type} key={key} />
              ))}
          </div>
        </div>
      </div>
      <SupportFooter />
      <SupportUserDetailSheet user={selectedChat.user} />
    </div>
  );
}
